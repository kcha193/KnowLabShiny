...
simulateKnowLab <- function(Simmodule, simenv) {
  ...
  simulate_psychosocial_factors <- function() {
    ...
    if (iteration <= 13) {
      ...
      chres <- adjustContVar(chres, "chres", propens=chresPropensities[,-ncol(chresPropensities)], simenv = simenv, iteration = iteration)
    } else {
      #iteration >13
      chres <- NAs
    }
  ...
  simulate_employment <- function() {
    if (iteration<=13) {
      ...
      #scenarios
      mhrswrk <- adjustContVar(mhrswrk, "mhrswrk", propens=mhrswrkPropensities[,-ncol(mhrswrkPropensities)], simenv = simenv, iteration = iteration)
      checkNAs(mhrswrk)
